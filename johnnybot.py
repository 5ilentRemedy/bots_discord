...
@bot.event
async def on_ready():
    await bot.change_presence(activity=discord.Streaming(type=discord.ActivityType.streaming, name='Burning Arasaka Tower', url='https://www.youtube.com/watch?v=AN1RJF55NXI'))
...
async def shop(ctx):
    print("shop-list")
    ID=ctx.author.id
    feedbackshoplist="List of available items - please buy them using buy comm "
    await ctx.reply(feedbackshoplist)
...
async def on_message(message):
    ...
    johnny_quotes = [
    'Mhm.','Widzimy się w Afterlife',
]

    if message.content == '.johnny':
        response = random.choice(johnny_quotes)
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        await ctx.send("Sorry chom, you have to wait for " f"{round(error.retry_after/60,0)} minutes to use that command")
